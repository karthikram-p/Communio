...
const Post = ({ post }) => {
    ...
    // Saved posts logic
    const { data: savedPosts = [] } = useQuery({ queryKey: ["savedPosts"] });
	const isSaved = savedPosts.some((p) => (typeof p === "object" ? p._id === post._id : p === post._id));

    const { mutate: savePost, isPending: isSaving } = useMutation({
        mutationFn: async () => {
            const res = await fetch(`/api/posts/save/${post._id}`, { method: "POST" });
    ...
    const { mutate: unsavePost, isPending: isUnsaving } = useMutation({
        mutationFn: async () => {
            const res = await fetch(`/api/posts/unsave/${post._id}`, { method: "POST" });
            const data = await res.json();
            if (!res.ok) throw new Error(data.error || "Something went wrong");
            return data;
        },
    ...
    const handleRepost = () => {
        if (isReposting || isMyPost) return;
        repost();
    };
                    ...
                    <div className='flex justify-between mt-3'>
                        <div className='flex gap-4 items-center w-2/3 justify-between'>
                            ...
                            <div
                                ...
                                title={isMyPost ? "You can't repost your own post" : "Repost"}
                            >
                                {isReposting ? (
                                ...
                                <span className='text-sm text-slate-500 group-hover:text-green-500'>Repost</span>
                            </div>
                        ...
                        <div className='flex w-1/3 justify-end gap-2 items-center'>
                            ...
                            {(isSaving || isUnsaving) && <LoadingSpinner size='sm' />}
                        </div>
                    </div>
                </div>
            </div>
